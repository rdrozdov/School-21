...
/*   Updated: 2022/02/08 18:33:13 by gcolene          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*my_read(char *reminder, int fd)
{
	...
	if (!buf)
		return (NULL);
	while (byte_was_read && !ft_strchr_gnl(reminder, '\n'))
	{
...
char	*get_result(char *reminder)
{
	...
	if (reminder[counter] == '\n')
		counter++;
	buf = ft_substr_gnl(reminder, 0, counter);
	return (buf);
}
...
char	*get_reminder(char *reminder)
{
	...
	buf = ft_substr_gnl(reminder, counter, (ft_strlen_gnl(reminder) - counter));
	free(reminder);
	return (buf);
}
