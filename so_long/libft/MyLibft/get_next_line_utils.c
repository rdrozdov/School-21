...
/*   Updated: 2022/02/16 16:59:24 by gcolene          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_strjoin_gnl(char *s1, char *s2)
{
	...
	if (!s2)
		return (NULL);
	count1 = 0;
	str = (char *)malloc (ft_strlen_gnl((char *) s1) + \
...
unsigned int start, size_t len)
{
	...
	count = 0;
	while (count < len && start < (unsigned int) ft_strlen_gnl((char *) s) \
	&& s[start])
	{
		str[count] = s[start++];
		count++;
